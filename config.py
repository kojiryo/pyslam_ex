...
class Config(object):
    ...
    def set_lib(self, lib_name, prepend=False):
        if lib_name in self.lib_paths:
            lib_paths = [e.strip()
                         for e in self.lib_paths[lib_name].split(',')]
    ...
    def get_dataset_settings(self):
        ...
        self.dataset_settings['base_path'] = os.path.join(
            __location__, self.dataset_path)
        #print('dataset_settings: ', self.dataset_settings)

    # get camera settings
    def get_cam_settings(self):
        self.cam_settings = None
        self.settings_doc = __location__ + '/' + \
            self.config_parser[self.dataset_type]['cam_settings']
        if(self.settings_doc is not None):
            with open(self.settings_doc, 'r') as stream:
                try:
                    self.cam_settings = yaml.load(
