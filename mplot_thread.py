...
kPlotSleep = 0.04
kVerbose = False
# from https://docs.python.org/3/library/threading.html#threading.Thread.daemon
kSetDaemon = True
...
class Mplot2d:
    def __init__(self, xlabel='', ylabel='', title=''):
        ...
        self.queue = Queue()
        self.vp = Process(target=self.drawer_thread, args=(
            self.queue, mp_lock, self.key, self.is_running,))
    ...
    def drawer_refresh(self, queue, lock):
        while not queue.empty():
            ...
            if name in self.handle_map:
                ...
                handle, = self.ax.plot(
                    xy_signal[0], xy_signal[1], c=color, marker=marker, label=name)
    ...
    def savefigimage(self):
        self.fig.savefig('matches.png')
    # +++
