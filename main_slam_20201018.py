...
def trial(dataset, groundtruth, cam, work_dir_path, current_trial_count, ** tracker_config):
    # *** for each trials ***
    try:
...
if __name__ == "__main__":
    ...
    for feature_name, tracker_config in pbar1:
        ...
        for current_trial_count in pbar2:
            ...
            while True:
                gc.collect()
                bTrial = trial(dataset, groundtruth, cam, work_dir_path, current_trial_count, **tracker_config)
                write_refcount(bTrial, 'refcount_bTrial.log', 'case0')
                if bTrial is True:
                    write_refcount(bTrial, 'refcount_bTrial.log', 'case1')
                    del bTrial
                    break  # while True:
                else:
                    write_refcount(bTrial, 'refcount_bTrial.log', 'case2')
                    del bTrial
                    continue  # while True:
