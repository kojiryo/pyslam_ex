...
# +++
# kLogKFposeToFile = True  # added for getting keyframe traj.
# +++
...
class TrackingHistory(object):
    def __init__(self):
        # list of relative frame poses as g2o.Isometry3d() (see camera_pose.py)
...
class Slam(object):
    def __init__(self, camera, feature_tracker, groundtruth=None):
        ...
        # not actually used here; could be used for evaluating performances
        self.groundtruth = groundtruth
        # +++
        # FrameBase._id = 0  # initialize Frame._id = 0 for each  trials
        # del FrameBase._id
        # +++
    ...
    def quit(self):
        ...
        # +++
        # del self.camera, self.map, self.local_mapping, self.groundtruth, self.tracking
        # gc.collect()
        # +++
...
class Tracking(object):
    def __init__(self, system):
        ...
        # current number of matched map points (matched and found valid in current pose optimization)
        self.num_matched_map_points = None
        # number of tracked points in k_ref (considering a minimum number of observations)
        ...
        # reference keyframe (in general, different from last keyframe depending on the used approach)
        self.kf_ref = None
        ...
        # set this to True if you want to print timings
        self.timer_verbose = kTimerVerbose
        self.timer_main_track = TimerFps(
            'Track', is_verbose=self.timer_verbose)
        self.timer_pose_opt = TimerFps(
            'Pose optimization', is_verbose=self.timer_verbose)
        ...
        self.timer_pose_est = TimerFps(
            'Ess mat pose estimation', is_verbose=self.timer_verbose)
        self.timer_frame = TimerFps('Frame', is_verbose=self.timer_verbose)
        self.timer_seach_map = TimerFps(
            'Search map', is_verbose=self.timer_verbose)

        self.init_history = True
        self.poses = []       # history of poses
        self.t0_est = None    # history of estimated translations
        # history of ground truth translations (if available)
        self.t0_gt = None
        ...
        # not actually used here; could be used for evaluating performances
        self.groundtruth = system.groundtruth

        if kLogKFinfoToFile:
            self.kf_info_logger = Logging.setup_file_logger(
                'kf_info_logger', 'kf_info.log', formatter=Logging.simple_log_formatter)

        # self.kf_pose_logger = None

# +++
    def print_tracking_history(self, filename):
        with open(file=filename, mode='w') as f:
            # self.kf_pose_logger = Logging.setup_file_logger(name='kf_pose_logger', log_file=filename, mode='+w', formatter=Logging.simple_log_formatter)
            kfs = self.map.get_keyframes()
            if kfs is not None:  # if kfs is void list, output brank txt file
                for kf in kfs:
                    # note: see camera_pose.py
                    # print('kf.Rwc: ', kf.Rwc)
                    quaternion_wc = Quaternion(matrix=kf.Rwc)  # (camera frame w.r.t. world origin)
                    # print('quaternion_wc: ', quaternion_wc)
                    # self.kf_pose_logger.info("{0} {1} {2} {3} {4} {5} {6} {7}"  # timestamp, x, y, z, qw, qx, qy, qz
                    #                         .format(kf.timestamp, kf.Ow[0], kf.Ow[1], kf.Ow[2], quaternion_wc[0], quaternion_wc[1], quaternion_wc[2], quaternion_wc[3]))
...
# +++
    ...
    def estimate_pose_by_fitting_ess_mat(self, f_ref, f_cur, idxs_ref, idxs_cur):
        ...
        if self.num_inliers < kNumMinInliersEssentialMat:
            ...
            # override translation with ref frame translation
            tcw = f_ref.pose[:3, 3]
        ...
        if use_search_frame_by_projection:
            ...
            if kDebugDrawMatches and True:
                img_matches = draw_feature_matches(f_ref.img, f_cur.img,
                                                   ...
                                                   horizontal=False)
                cv2.imshow(
                    'tracking frame by projection - matches', img_matches)
                cv2.waitKey(1)
            ...
            if self.num_matched_kps < Parameters.kMinNumMatchedFeaturesSearchFrameByProjection:
                ...
                if not self.pose_is_ok or self.num_matched_map_points < kNumMinInliersPoseOptimizationTrackFrame:
                    Printer.red(
                        'failure in tracking previous frame, # matched map points: ', self.num_matched_map_points)
                    self.pose_is_ok = False
    ...
    def track_reference_frame(self, f_ref, f_cur, name=''):
        ...
        if not self.pose_is_ok or self.num_matched_map_points < kNumMinInliersPoseOptimizationTrackFrame:
            f_cur.remove_frame_views(idxs_cur)
            f_cur.reset_points()
            Printer.red('failure in tracking reference %d, # matched map points: %d' % (
    ...
    def track_local_map(self, f_cur):
        ...
        num_found_map_pts, reproj_err_frame_map_sigma, matched_points_frame_idxs = search_map_by_projection(self.local_points, f_cur,
                                                                                                            # Parameters.kMaxReprojectionDistanceMap,
        ...
        if not self.pose_is_ok or self.num_matched_map_points < kNumMinInliersPoseOptimizationTrackLocalMap:
            Printer.red('failure in tracking local map, # matched map points: ',
                        self.num_matched_map_points)
            self.pose_is_ok = False
    ...
    def need_new_keyframe(self, f_cur):
        # print('in need_new_keyframe case 0')
        num_keyframes = self.map.num_keyframes()
        # print('in need_new_keyframe case 1')
        nMinObs = kNumMinObsForKeyFrameDefault
        # print('in need_new_keyframe case 2')
        if num_keyframes <= 2:
            # print('in need_new_keyframe case 3')
        ...
        # print('in need_new_keyframe case 13')
        # condition 2: more than "min_frames_between_kfs" have passed and local mapping is idle
        cond2 = (f_cur.id >= (self.kf_last.id +
                              self.min_frames_between_kfs)) & is_local_mapping_idle
        # cond2 = (f_cur.id >= (self.kf_last.id + self.min_frames_between_kfs))

        # print('in need_new_keyframe case 14')
        # condition 3: few tracked features compared to reference keyframe
        cond3 = (num_f_cur_tracked_points < num_kf_ref_tracked_points *
                 Parameters.kThNewKfRefRatio) and (num_f_cur_tracked_points > Parameters.kNumMinPointsForNewKf)

        # print('in need_new_keyframe case 15')
        # print('KF conditions: %d %d %d' % (cond1, cond2, cond3) )
        ret = (cond1 or cond2) and cond3

        # print('in need_new_keyframe case 16')
        if ret:
            # print('in need_new_keyframe case 17')
            if is_local_mapping_idle:
                # print('in need_new_keyframe case 18')
                return True
            else:
                # print('in need_new_keyframe case 19')
                self.local_mapping.interrupt_optimization()
                # print('in need_new_keyframe case 20')
                if True:
                    # print('in need_new_keyframe case 21')
                    if local_mapping_queue_size <= 3:
                        # print('in need_new_keyframe case 22')
                        return True
                    else:
                        # print('in need_new_keyframe case 23')
                        return False
                else:
                    # print('in need_new_keyframe case 24')
                    return False
        else:
            # print('in need_new_keyframe case 25')
            return False
    ...
    def track(self, img, frame_id, timestamp=None):
        ...
        if self.state == SlamState.NOT_INITIALIZED:
            ...
            if intializer_is_ok:
                ...
                # add first frame in map and update its frame id
                self.map.add_frame(kf_ref)
                # add second frame in map and update its frame id
                ...
                # add second keyframe in map and update its kid
                ...
                # after initialization you cannot use motion model for next frame pose prediction (time ids of initialized poses may not be consecutive)
        ...
        with self.map.update_lock:
            ...
            if kUseMotionModel and self.motion_model.is_ok:
                ...
                self.f_ref.update_pose(
                    self.tracking_history.relative_frame_poses[-1] * self.f_ref.kf_ref.isometry3d)
        ...
        with self.map.update_lock:
            ...
            if self.pose_is_ok:   # if tracking was successful
                ...
                # update motion model
                self.motion_model.update_pose(
                    timestamp, f_cur.position, f_cur.quaternion)

                f_cur.clean_vo_map_points()
                # +++check
                print('DONE: f_cur.clean_vo_map_points()')
                # +++check
                # do we need a new KeyFrame?
                need_new_kf = self.need_new_keyframe(f_cur)
                # +++check
                print('DONE: need_new_kf = self.need_new_keyframe(f_cur)')
                # +++check
    ...
    #             [self.trueX-self.t0_gt[0], self.trueY-self.t0_gt[1], self.trueZ-self.t0_gt[2]])
