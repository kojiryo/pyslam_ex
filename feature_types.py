...
class FeatureDetectorTypes(Enum):
    ...
    # interface for ORB-SLAM2 features (ORB + spatial keypoint filtering)
    ORB2 = 7
    ...
    # [end-to-end] joint detector-descriptor - "SuperPoint: Self-Supervised Interest Point Detection and Description"
    SUPERPOINT = 11
    AGAST = 12  # "AGAST Corner Detector: faster than FAST and even FAST-ER"
    # "Good Features To Track" (it includes SHI-TOMASI and HARRIS methods)
    GFTT = 13
    ...
    # [end-to-end] joint detector-descriptor - "D2-Net: A Trainable CNN for Joint Detection and Description of Local Features".
    D2NET = 18
    # [end-to-end] joint detector-descriptor - "Large-Scale Image Retrieval with Attentive Deep Local Features".
    DELF = 19
    # [end-to-end] only with CONTEXTDESC descriptor - "ContextDesc: Local Descriptor Augmentation with Cross-Modality Context"
    CONTEXTDESC = 20
    # [end-to-end] joint detector-descriptor - "LF-Net: Learning Local Features from Images"
    LFNET = 21
    # [end-to-end] joint detector-descriptor - "R2D2: Repeatable and Reliable Detector and Descriptor"
    R2D2 = 22
...
class FeatureDescriptorTypes(Enum):
    ...
    # [binary] only with KAZE or AKAZE detectors - "Fast Explicit Diffusion for Accelerated Features in Nonlinear Scale Spaces"
    AKAZE = 8
    FREAK = 9   # [binary] only descriptor - "FREAK: Fast retina keypoint"
    # [end-to-end] only with SUPERPOINT detector - "SuperPoint: Self-Supervised Interest Point Detection and Description"
    SUPERPOINT = 10
    TFEAT = 11  # only descriptor - " Learning local feature descriptors with triplets and shallow convolutional neural networks"
    # [binary] only descriptor - "Learning Image Descriptors with Boosting"
    BOOST_DESC = 12
    DAISY = 13  # only descriptor - "Daisy: An efficient dense descriptor applied to wide baseline stereo"
    # [binary] only descriptor - "LATCH: Learned Arrangements of Three Patch Codes"
    LATCH = 14
    # [binary] only descriptor - "Locally uniform comparison image descriptor" - (it requires a color image) <-- !N.B.: not producing good results!
    LUCID = 15
    ...
    # [end-to-end] joint detector-descriptor (only with SUPERPOINT detector) - "SOSNet:Second Order Similarity Regularization for Local Descriptor Learning"
    SOSNET = 19
    ...
    # [end-to-end] joint detector-descriptor (only with D2NET detector) - "D2-Net: A Trainable CNN for Joint Detection and Description of Local Features".
    D2NET = 22
    # [end-to-end] joint detector-descriptor (only with DELF detector) - "Large-Scale Image Retrieval with Attentive Deep Local Features".
    DELF = 23
    # [end-to-end] only with CONTEXTDESC detector - "ContextDesc: Local Descriptor Augmentation with Cross-Modality Context"
    CONTEXTDESC = 24
    # [end-to-end] joint detector-descriptor (only with LFNET detector) - "LF-Net: Learning Local Features from Images"
    LFNET = 25
    # [end-to-end] joint detector-descriptor (only with R2D2 detector) - "R2D2: Repeatable and Reliable Detector and Descriptor"
    R2D2 = 26
    # keynet descriptor is HARDNET (only with KEYNET detector) - "Key.Net: Keypoint Detection by Handcrafted and Learned CNN Filters"
    KEYNET = 27


class FeatureInfo(object):
    norm_type = dict()
    # initial reference max descriptor distances used by SLAM for locally searching matches around frame keypoints;
    max_descriptor_distance = dict()
    ...
    # ROOT_SIFT
    max_descriptor_distance[FeatureDescriptorTypes.ROOT_SIFT] = 0.9
    #
    ...
    # SUPERPOINT
    max_descriptor_distance[FeatureDescriptorTypes.SUPERPOINT] = 1.30
    #
    ...
    # BOOST_DESC
    max_descriptor_distance[FeatureDescriptorTypes.BOOST_DESC] = 100
    #
    ...
    # HARDNET
    max_descriptor_distance[FeatureDescriptorTypes.HARDNET] = 1.8
    #
    # unless GeodescFeature2D.quantize == True
    norm_type[FeatureDescriptorTypes.GEODESC] = cv2.NORM_L2
    # GEODESC
    max_descriptor_distance[FeatureDescriptorTypes.GEODESC] = 0.4
    #
    norm_type[FeatureDescriptorTypes.SOSNET] = cv2.NORM_L2
    # SOSNET
    max_descriptor_distance[FeatureDescriptorTypes.SOSNET] = 2
    #
    ...
    # LOGPOLAR
    max_descriptor_distance[FeatureDescriptorTypes.LOGPOLAR] = 3.2
    #
    ...
    #
    # unless ContextDescFeature2D.quantize == True
    norm_type[FeatureDescriptorTypes.CONTEXTDESC] = cv2.NORM_L2
    # CONTEXTDESC
    max_descriptor_distance[FeatureDescriptorTypes.CONTEXTDESC] = 1.6
    #
    ...
    # KEYNET
    max_descriptor_distance[FeatureDescriptorTypes.KEYNET] = 1.6
