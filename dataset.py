...
def dataset_factory(settings):
    ...
    if type == 'carla':
        dataset = CARLADataset(path, name, associations, DatasetType.CARLA)
    if type == 'euroc':
        dataset = EuRoCDataset(path, name, associations, DatasetType.EUROC)
...
class FolderDatasetParallel(Dataset):
    def __init__(self, path, name, fps=None, associations=None, type=DatasetType.VIDEO):
        ...
        # pass the folder status with the initialization
        self.q.put(self.folder_status)
...
class TumDataset(Dataset):
    ...
    def getImage(self, frame_id):
        ...
        if frame_id < self.max_frame_id:
            ...
            # img = cv2.imread(file) # change 20201127
            img = cv2.imread(file, cv2.IMREAD_GRAYSCALE)  # change 20201127
            self.is_ok = (img is not None)
...
class CARLADataset(Dataset):
    def __init__(self, path, name, fps=15, associations=None, type=DatasetType.VIDEO):
        super().__init__(path, name, 15, associations, type)
        print('fps: ', self.fps)
        self.Ts = 1. / self.fps
        self.skip = 1
        self.listing = []
    ...
    def getImage(self, frame_id):
        if self.i == self.maxlen:
            return (None, False)
        image_file = self.listing[self.i]
        # img = cv2.imread(image_file)  # always
        img = cv2.imread(image_file, cv2.IMREAD_GRAYSCALE)  # always
        self._timestamp += self.Ts
        self._next_timestamp = self._timestamp + self.Ts
        if img is None:
            raise IOError('error reading file: ', image_file)
...
class EuRoCDataset(Dataset):
    def __init__(self, path, name, fps=20, associations=None, type=DatasetType.EUROC):
        ...
        self.maxlen = 1000000
        print('Processing Image Directory Input')
        self.listing = glob.glob(os.path.join(path, self.name, 'mav0/cam0/data/*.png'))
        self.listing.sort()
        self.listing = self.listing[::self.skip]
        # print('list of files: ', self.listing)
        self.maxlen = len(self.listing)
        ...
        if self.maxlen == 0:
            raise IOError('No images were found in folder: ', path)
        self.max_frame_id = self.maxlen
        print('self.max_frame_id: ', self.max_frame_id)
        self._timestamp = 0.
    ...
    def getImage(self, frame_id):
        ...
        if frame_id < self.max_frame_id:
            ...
            # img = cv2.imread(image_file)  # always
            img = cv2.imread(image_file, cv2.IMREAD_GRAYSCALE)  # always
            image_file_name = float(os.path.splitext(os.path.basename(image_file))[0])
            # print(image_file_name)
            self._timestamp = float(round(image_file_name / 10**9, 6))
            if frame_id + 1 < self.max_frame_id:
                self._next_timestamp = float(round(float(os.path.splitext(os.path.basename(self.listing[self.i + 1]))[0]) / 10**9, 6))
            else:
                self._next_timestamp = self.timestamps
            # self._timestamp += self.Ts
            # self._next_timestamp = self._timestamp + self.Ts
    ...
    # float(round(float(timestamp) / 10**9, 6)
