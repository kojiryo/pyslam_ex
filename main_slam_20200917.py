...
if __name__ == "__main__":
            ...
            if img is not None:
                ...
                if matched_points_plt is not None:
                    if slam.tracking.num_matched_kps is not None:
                        matched_kps_signal = [
                            img_id, slam.tracking.num_matched_kps]
                    ...
                    if slam.tracking.num_matched_map_points is not None:
                        # valid matched map points (in current pose optimization)
                        valid_matched_map_points_signal = [
                            img_id, slam.tracking.num_matched_map_points]
                        matched_points_plt.draw(
                    ...
                    if slam.tracking.num_kf_ref_tracked_points is not None:
                        kf_ref_tracked_points_signal = [
                            img_id, slam.tracking.num_kf_ref_tracked_points]
                        matched_points_plt.draw(
                            kf_ref_tracked_points_signal, '# $KF_{ref}$  tracked pts', color='c')
                    if slam.tracking.descriptor_distance_sigma is not None:
                        descriptor_sigma_signal = [
                            img_id, slam.tracking.descriptor_distance_sigma]
                        matched_points_plt.draw(
    ...
    # +++
    slam.tracking.print_tracking_history()

    # +++
    if kLogDurToFile:
        dur_logger = Logging.setup_file_logger(
            'dur_logger', 'dur.log', formatter=Logging.simple_log_formatter)
        dur_logger.info(tracker_config)
        dur_logger.info('SlamState: %s' % (slam.tracking.state.name))
        if duration_list is not None:
            mean_duration = sum(duration_list) / len(duration_list)
            dur_logger.info('mean_duration[s]: %f' % (mean_duration))
    # +++

    print('tracker_config: ', tracker_config)
    if duration_list is not None:
        mean_duration = sum(duration_list) / len(duration_list)
        print('mean_duration[s]: ', mean_duration)
    # +++
    # +++
    # if matched_points_plt is not None:
