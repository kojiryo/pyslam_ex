...
# +++
        if kLogKFposeToFile:
            self.kf_pose_logger = Logging.setup_file_logger(
                'kf_pose_logger', 'kf_pose.log', formatter=Logging.simple_log_formatter)
# +++
    ...
    def need_new_keyframe(self, f_cur):
        ...
        Printer.purple('F(%d) #points: %d, KF(%d) #points: %d ' % (
            f_cur.id, num_f_cur_tracked_points, self.kf_ref.id, num_kf_ref_tracked_points))

        if kLogKFinfoToFile:
            self.kf_info_logger.info('F(%d) #points: %d, KF(%d) #points: %d ' % (
                f_cur.id, num_f_cur_tracked_points, self.kf_ref.id, num_kf_ref_tracked_points))
...
# +++
# added for getting keyframe traj.
                    if kLogKFposeToFile:
                        # print(type(kf_new.quaternion))
                        # print(kf_new.quaternion)
                        self.kf_pose_logger.info(
                            "{0} {1} {2} {3}".format(
                                kf_new.timestamp, kf_new.position[0], kf_new.position[1], kf_new.position[2]))  # , str(kf_new.quaternion))
                        # self.kf_pose_logger.info(
                        #     "{0} {1} {2} {3} {4}".format(
                        #         kf_new.timestamp, kf_new.position[0], kf_new.position[1], kf_new.position[2], kf_new.quaternion))  # , str(kf_new.quaternion))
# +++
