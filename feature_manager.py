...
if False:
    L2NetKerasFeature2D = import_from(
        'feature_l2net_keras', 'L2NetKerasFeature2D')  # not used at present time
...
ContextDescFeature2D = import_from(
    'feature_contextdesc', 'ContextDescFeature2D')
...
class KeyPointFilterTypes(Enum):
    NONE = 0
    # sat the number of features (keep the best N features: 'best' on the basis of the keypoint.response)
    SAT = 1
    ...
    # Distribute keypoints by using a octree (as a matter of fact, a quadtree): from ORBSLAM2
    OCTREE_NMS = 4
    GRID_NMS = 5      # NMS by using a grid


def feature_manager_factory(num_features=kNumFeatureDefault,
                            # number of pyramid levels or octaves for detector and descriptor
...
class FeatureManager(object):
    def __init__(self, num_features=kNumFeatureDefault,
        ...
        if cv2.__version__.split('.')[0] == '3':
            ...
            BoostDesc_create = import_from(
                'cv2', 'xfeatures2d_BoostDesc', 'create')
            ...
            STAR_create = import_from(
                'cv2', 'xfeatures2d_StarDetector', 'create')
