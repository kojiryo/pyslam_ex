...
# sys.path.append('/home/rkojima/Git/pyslam_dataprocessing/LINEnotify.py')
# import LINEnotify
            ...
            while self.dataset.isOk():
                ...
                if self.img is not None:
                    ...
                    if not self.bIniDone:
                        if self.slam.tracking.state == self.SlamState.OK:
                            self.bIniDone = True
                            if self.img_id <= self.frame_id_iniTh:
                                self.bSuccess = True
                    ...
                    if self.bLost is False:
                        if self.slam.tracking.state == self.SlamState.LOST:
                            self.bLost = True
                        ...
                        if (self.slam.tracking.state == self.SlamState.OK) and (self.img_id == self.num_frames - 1):  # last frame in sequence
                            self.frame_id_end = self.img_id

                    # Recover from the tracking lost
                    if (self.bLost is True):
                        if self.slam.tracking.state == self.SlamState.OK:
                            self.bLost = False
                            self.frame_id_end = -1
                    ...
                    if kSaveFrameImage is True:
                        # self.img_gray = cv2.cvtColor(self.img, cv2.COLOR_BGR2GRAY)
                        self.img_draw_o = self.slam.map.draw_feature_trails(self.img)
                        self.img_draw_resize = cv2.resize(self.img_draw_o, None, fx=0.5, fy=0.5)
                        cv2.imwrite(os.path.join(self.work_dir_path, 'frameimage', str(self.current_trial_count).zfill(4), str(self.img_id).zfill(6) + '.jpg'), self.img_draw_resize)
...
if __name__ == "__main__":
    ...
    for feature_name, tracker_config in pbar1:
        ...
        for current_trial_count in pbar2:
            ...
            if kSaveFrameImage is True:
                os.makedirs(os.path.join(work_dir_path, 'frameimage', str(current_trial_count).zfill(4)), exist_ok=True)
            ...
            # sys.exit(1)  # shell retry
            # LINEnotify.send_line_notify('{},{},{},{}'.format(dataset_name, sequence_name, feature_name, trial_count))
            os._exit(1)
        # gc.collect()
        # with open(file='gc_garbage.log', mode='a') as f:
